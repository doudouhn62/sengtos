#!/bin/bash
#cat /etc/ssh/sshd_config
#rm -rf /root/.ssh
#rm -rf /home/$your_user/.ssh
rm -rf .ssh

mkdir .ssh

chmod 700 .ssh

cd .ssh

echo -e "\n" | ssh-keygen -t rsa -b 4096 -N ""
#ssh-keygen -t rsa -b 4096 -N ""
#printf '\n' | ssh-keygen -t rsa -b 4096 -N ''

cat id_rsa.pub > authorized_keys

chmod 600 authorized_keys

cd

#sed -i -e 's@^.\{,1\}Port.*@Port 22@g' /etc/ssh/sshd_config
#sed -i -e 's@.*Port.*@Port 22@g' /etc/ssh/sshd_config

sed -i -e 's@\(.*X11Forwarding.*\)@#\1@g' /etc/ssh/sshd_config

echo -e "RSAAuthentication yes\nPubkeyAuthentication yes" >> /etc/ssh/sshd_config

echo -e "ClientAliveInterval 60\nClientAliveCountMax 3\nMaxAuthTries 10" >> /etc/ssh/sshd_config

systemctl restart sshd


read -p "请设置你的linux用户名:" your_user
read -p "请设置你的ssh端口号:" ssh_port

if [ "$ssh_port" -gt 0 ];then
	if [ "$ssh_port" -le 59999 -a "$ssh_port" -ge 10001 ];then
		echo "你设置的linux用户名是:$your_user"
		echo "你设置的ssh端口号是:$ssh_port"
	else
		echo "你设置的ssh端口号不属于用户端可用的范围，请重新输入"
		exit 5
	fi
else
	echo "输入有误，请重新输入全数字的ssh端口号"
	exit 6
fi


sed -i -e '/#Port/aPort '$ssh_port'' /etc/ssh/sshd_config


useradd $your_user


cp -a /root/.ssh /home/$your_user
#mkdir -pv /home/$your_user/.ssh
#cp /root/.ssh/* /home/$your_user/.ssh

chmod 700 /home/$your_user/.ssh

chmod 600 /home/$your_user/.ssh/authorized_keys

chown -R $your_user:$your_user /home/$your_user/.ssh


sed -i -e '/^root/a'$your_user' ALL=(ALL) ALL' /etc/sudoers


echo guangshui | passwd --stdin $your_user


#使用自定义用户名及端口和密钥登录
#sed -i -e 's@^PasswordAuthentication.*@PasswordAuthentication no@g' -e 's@^.*PermitRootLogin.*@PermitRootLogin no@g' /etc/ssh/sshd_config ; systemctl restart sshd

#恢复使用root和密码登录
#sed -i -e 's@^PasswordAuthentication.*@PasswordAuthentication yes@g' -e 's@^.*PermitRootLogin.*@PermitRootLogin yes@g' /etc/ssh/sshd_config ; systemctl restart sshd
